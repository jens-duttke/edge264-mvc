...
- cabac_all_incomplete.264: 24 unfinished two-macroblock pictures generated by
  ...
  deterministic 16x16 I_PCM output. Sixteen pictures fill the DPB while none is
  ready for normal output. `check-edge264-test-liveness` then exercises the
  tool's caller-side progress guard, which forces an end-of-stream drain after
  repeated zero-progress `ENOBUFS`. The guard must set `flushing` before feeding
  the sentinel; otherwise the normal NAL fullness gate rejects the sentinel
  forever. The focused harness checks all 16 cropped output frames through both
  regular-file and stdin paths, in single- and multithreaded modes.
