...
| Round SSE temporal-direct motion vectors with the spec's +128 bias (a signed `DistScaleFactor` sign-extended into the packed rounding constant, biasing the round to +129 for every negative scale factor - the ordinary "current picture between the collocated picture and its reference" case), so B-slice and MVC inter-view direct motion vectors on the shipped x86/SSE build are bit-exact to the NEON / WASM / scalar backends | edge264-mvc |
