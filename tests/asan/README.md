...
a log callback. Built with -fsanitize=address; run under a timeout. A fixture
flagged `clean` in the manifest is a VALID stream that must decode with zero
EBADMSG (asserted by the harness); the others are crafted/malformed and only
guard against sanitizer aborts and CPU-burn.

- sei_recovery_point.264 (clean): a 26-byte VALID stream (SPS + PPS + a tiny
  recovery_point SEI, generated by tests/gen_sei_recovery_point.py) whose SEI
  RBSP `06 01 c4 80` is byte-identical to the one that closes every base-view
  access unit on real 3D Blu-rays. parse_sei skipped an unhandled message by
  bounding the byte-skip on gb.CPB (the reader's REFILL pointer), but a NAL this
  small sits entirely in the bit cache so CPB == end already and the skip
  advanced nothing: the payload byte was re-read as a bogus follow-on message,
  the reader was left mid-syntax, rbsp_end failed and the SEI NAL returned
  EBADMSG - failing the whole (valid) stream, on essentially every access unit,
  in the logging/trace path. The fix bounds the skip by the cache-aware bit count
  (the rbsp_end formula). The harness asserts this fixture decodes with zero
  EBADMSG; the buggy skip returns EBADMSG on the SEI and fails it.
- sei_short_payload.264 (clean): a 26-byte VALID stream (SPS + PPS + a pic_timing
  SEI, generated by tests/gen_sei_short_payload.py) whose SEI RBSP `01 01 80 80`
  declares payloadType 1 (pic_timing), payloadSize 1. With no VUI (no HRD /
  pic_struct) parse_pic_timing reads zero bits and succeeds, so the one payload
  byte is unconsumed reserved data. parse_sei's success path only byte-ALIGNED
  the reader instead of advancing to start + payloadSize (as the skip path does),
  so the reserved byte was left in the stream, re-read as a bogus follow-on
  message; the reader overran the rbsp_trailing, rbsp_end failed and the SEI NAL
  returned EBADMSG - failing a valid stream (logging/trace path). The fix advances
  to start + payloadSize on every message, success or not. The harness asserts
  this fixture decodes with zero EBADMSG; the buggy byte-align fails it.
